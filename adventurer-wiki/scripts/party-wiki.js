...
Hooks.once("ready", () => {
  ...
  game.socket.on(SOCKET_EVENT, async (payload) => {
    switch (payload.action) {
      ...
      case "refresh": { refreshAllWikiApps(); break; }
      case "categoriesChanged": { refreshAllWikiApps(); break; }
      case "editingStart": {
        activeEditors.set(payload.entryId, { userName: payload.userName, userId: payload.userId });
        refreshAllWikiApps();
        break;
      }
      case "editingStop": {
        const cur = activeEditors.get(payload.entryId);
        if (cur?.userId === payload.userId) { activeEditors.delete(payload.entryId); refreshAllWikiApps(); }
        break;
      }
    }
  });
...
function processEntryLinks(html, allEntries) {
  ...
  return html.replace(/\[\[([^\]]+)\]\]/g, (_match, rawTitle) => {
    ...
    if (linked) return `<a class="wiki-entry-link" data-id="${linked.id}">${title}</a>`;
...
async function saveEntries(entries) {
  ...
  if (!activeGM) {
    ui.notifications.warn("Adventurer Wiki: A GM must be connected to save. Your work is still in the editor — please try again once a GM joins.");
    return false;
  }
...
function addWikiButton() {
  ...
  if (!list) { console.log("Adventurer Wiki | Controls list not found in DOM yet."); return; }
  ...
  if (isV12) {
    ...
    btn.addEventListener("click", (e) => { e.preventDefault(); e.stopPropagation(); new PartyWikiApp().render(true); });
    li.appendChild(btn);
  } else {
    ...
    li.addEventListener("click", (e) => { e.preventDefault(); e.stopPropagation(); new PartyWikiApp().render(true); });
  }
  list.appendChild(li);
...
class PartyWikiApp extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  async _prepareContext(_options) {
    ...
    const current = this._selectedEntry ? (entries.find(e => e.id === this._selectedEntry) ?? null) : null;
    const _TextEditor = foundry.applications?.ux?.TextEditor?.implementation ?? TextEditor;
    const enrichedContent = current?.content ? await _TextEditor.enrichHTML(current.content, { async: true }) : null;
    const pendingDeletions = entries.filter(e => e.pendingDelete).length;
    const cats = getCategories();
    if (!cats.find(c => c.id === this._activeCat)) this._activeCat = cats[0]?.id ?? null;
    const entriesWithStatus = filtered.map(e => {
      ...
      return { ...e, beingEditedBy: activeEditors.get(e.id)?.userName ?? null,
        categoryLabel: q ? (cats.find(c => c.id === e.category)?.label ?? e.category) : null, bodyMatch };
    });
    const currentEditor = current ? (activeEditors.get(current.id)?.userName ?? null) : null;
    const categoriesWithCount = cats.map(cat => ({ ...cat, count: entries.filter(e => e.category === cat.id).length }));
    const updatedAtFormatted = current?.updatedAt ? new Date(current.updatedAt).toLocaleString(undefined, { month: "short", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit" }) : null;
    ...
    const formattedComments = (current?.comments ?? []).map(c => ({
      ...c, createdAtFormatted: new Date(c.createdAt).toLocaleString(undefined, { month: "short", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit" }),
      canDelete: isGM || c.userId === game.user.id,
    }));
    const gmOnline = game.user.isGM || !!game.users.find(u => u.isGM && u.active);
    return { categories: categoriesWithCount, activeCat: this._activeCat, entries: entriesWithStatus, current, currentEditor,
      enrichedContent: enrichedContentLinked, gmNotes, formattedComments, hasComments: formattedComments.length > 0,
      isGM, gmOnline, searchQuery: this._searchQuery, isSearching: !!this._searchQuery,
      noEntries: filtered.length === 0, pendingDeletions, updatedAtFormatted };
  }
  ...
  _onRender(_context, _options) {
    ...
    if (this._searchQuery) {
      ...
      if (searchEl) { searchEl.focus(); const len = searchEl.value.length; searchEl.setSelectionRange(len, len); }
    }
  }
  ...
  _attachDelegatedListeners() {
    ...
    const _click = async (e) => {
      ...
      if (catTab) { this._activeCat = catTab.dataset.cat; this._selectedEntry = null; this.render({ force: true }); return; }
      const entryItem = e.target.closest(".wiki-entry-item");
      if (entryItem) { this._selectedEntry = entryItem.dataset.id; this.render({ force: true }); return; }
      if (e.target.closest(".wiki-btn-toggle-hidden")) {
        if (!this._selectedEntry || !game.user.isGM) return;
        const entries = getEntries(); const idx = entries.findIndex(en => en.id === this._selectedEntry);
        if (idx === -1) return; entries[idx] = { ...entries[idx], hidden: !entries[idx].hidden }; await saveEntries(entries); return;
      }
      if (e.target.closest(".wiki-btn-new")) { new WikiEntryEditor({ category: this._activeCat }, this).render(true); return; }
      if (e.target.closest(".wiki-btn-settings")) { new WikiCategorySettings().render(true); return; }
      if (e.target.closest(".wiki-btn-edit")) {
        if (!this._selectedEntry) return;
        const entry = getEntries().find(en => en.id === this._selectedEntry);
        if (entry) new WikiEntryEditor(entry, this).render(true); return;
      }
      if (e.target.closest(".wiki-btn-request-delete")) {
        if (!this._selectedEntry) return;
        const ok = await foundry.applications.api.DialogV2.confirm({ window: { title: "Request Deletion" }, content: "<p>Flag this entry for deletion? The GM will need to approve it.</p>" });
        if (!ok) return;
        const entries = getEntries().map(en => en.id === this._selectedEntry ? { ...en, pendingDelete: true } : en);
        await saveEntries(entries); return;
      }
      if (e.target.closest(".wiki-btn-cancel-delete")) {
        if (!this._selectedEntry) return;
        const entries = getEntries().map(en => en.id === this._selectedEntry ? { ...en, pendingDelete: false } : en);
        await saveEntries(entries); return;
      }
      if (e.target.closest(".wiki-btn-delete")) {
        if (!this._selectedEntry) return;
        const ok = await foundry.applications.api.DialogV2.confirm({ window: { title: "Delete Entry" }, content: "<p>Permanently delete this entry? This cannot be undone.</p>" });
        if (!ok) return;
        const entries = getEntries().filter(en => en.id !== this._selectedEntry);
        this._selectedEntry = null; await saveEntries(entries); return;
      }
      const entryLink = e.target.closest(".wiki-entry-link[data-id]");
      if (entryLink) { e.preventDefault(); this._selectedEntry = entryLink.dataset.id; this.render({ force: true }); return; }
      const missingLink = e.target.closest(".wiki-entry-link-missing");
      if (missingLink) { e.preventDefault(); ui.notifications.warn(`Adventurer Wiki: No entry found named "${missingLink.textContent}".`); return; }
      if (e.target.closest(".wiki-comment-submit")) { await this._submitComment(); return; }
      const commentDelBtn = e.target.closest(".wiki-comment-delete");
      if (commentDelBtn) { await this._deleteComment(commentDelBtn.dataset.commentId); return; }
    };

    const _input = (e) => {
      if (!inApp(e)) return;
      if (!e.target.matches(".wiki-search")) return;
      this._searchQuery = e.target.value; this.render({ force: true });
    };

    const _keydown = (e) => {
      if (!inApp(e)) return;
      if (!e.target.matches(".wiki-comment-input")) return;
      if (e.key === "Enter" && (e.ctrlKey || e.metaKey)) { e.preventDefault(); this._submitComment(); }
    };
  ...
  async _submitComment() {
    ...
    const entries  = getEntries(); const entryIdx = entries.findIndex(e => e.id === this._selectedEntry);
    if (entryIdx === -1) return;
    const comment = { id: generateId(), authorName: game.user.name, userId: game.user.id, text, createdAt: Date.now() };
  ...
  async close(options) {
    if (this._docListeners) {
      ...
      this._docListeners = null; this._listenersReady = false;
    }
    openWikiApps.delete(this); return super.close(options);
  }
}
...
class WikiEntryEditor extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  constructor(entry, wikiApp, options = {}) {
    super(options); this._entry = entry; this._wikiApp = wikiApp;
    this._broadcastedStart = false; this._pmEditor = null;
  }

  async _prepareContext(_options) {
    const entry = this._entry ?? { title: "", category: "lore", content: "" };
    return { entry, categories: getCategories(), isNew: !entry.id, isGM: game.user.isGM };
  }

  async _onRender(context, options) {
    const el = this.element;
    activeEditorApp = this;
    if (!this._hasOpened) { this._hasOpened = true; (this.bringToFront ?? this.bringToTop)?.call(this); }
    if (!this._broadcastedStart && this._entry?.id) {
      ...
      game.socket.emit(SOCKET_EVENT, { action: "editingStart", entryId: this._entry.id, userName: game.user.name, userId: game.user.id });
      refreshAllWikiApps();
    }
    ...
    if (!this._editorReady) {
      ...
      if (editorDiv && textarea) {
        ...
        el.querySelectorAll(".wiki-toolbar-btn[data-cmd]").forEach(btn => {
          btn.addEventListener("mousedown", (e) => {
            ...
            if (cmd === "formatBlock" && value && value !== "p") {
              ...
              if (cur === value.toLowerCase()) { document.execCommand("formatBlock", false, "p"); editorDiv.focus(); return; }
            }
            document.execCommand(cmd, false, value); editorDiv.focus();
          });
        });
        ...
        el.querySelector(".wiki-toolbar-btn-link")?.addEventListener("mousedown", (e) => {
          ...
          const range = sel.getRangeAt(0); const selected = range.toString(); const label = selected || "Entry Title";
          range.deleteContents();
          const node = document.createTextNode(`[[${label}]]`);
          range.insertNode(node);
          const start = 2; range.setStart(node, start); range.setEnd(node, start + label.length);
          sel.removeAllRanges(); sel.addRange(range); editorDiv.focus();
        });
        ...
        el.querySelector(".wiki-toolbar-btn-doodle")?.addEventListener("mousedown", (e) => {
          e.preventDefault();
          if (!game.user.can("FILES_UPLOAD")) {
            ui.notifications.warn("Adventurer Wiki: You need file upload permissions to insert doodles. Ask your GM to enable this in world settings.");
            return;
          }
        ...
        editorDiv.addEventListener("keydown", (e) => {
          if (e.key === "Enter" && (e.ctrlKey || e.metaKey)) { e.preventDefault(); this._handleSave(el.querySelector("form")); }
        });
      }

      if (game.user.isGM) {
        const gmNotesEl = el.querySelector(".wiki-gm-notes-input");
        if (gmNotesEl && !this._gmNotesReady) { this._gmNotesReady = true; gmNotesEl.value = this._entry?.gmNotes ?? ""; }
        const hiddenEl = el.querySelector(".wiki-hidden-checkbox");
        if (hiddenEl) hiddenEl.checked = this._entry?.hidden ?? false;
      }
    }

    el.querySelector("form")?.addEventListener("submit", async (e) => { e.preventDefault(); await this._handleSave(el.querySelector("form")); });
    el.querySelector(".wiki-btn-save")?.addEventListener("click", async (e) => { e.preventDefault(); await this._handleSave(el.querySelector("form")); });
  }
  ...
  async _handleSave(form) {
    ...
    if (!title) { ui.notifications.warn("Please enter a title for the entry."); return; }
    ...
    const entries = getEntries(); const now = Date.now();
    if (this._entry?.id) {
      const idx = entries.findIndex(e => e.id === this._entry.id);
      if (idx !== -1) entries[idx] = { ...entries[idx], title, category, content, updatedAt: now, updatedBy: game.user.name, pendingDelete: false, comments: entries[idx].comments ?? [], ...(game.user.isGM ? { gmNotes, hidden } : {}) };
    } else {
      entries.push({ id: generateId(), title, category, content, createdAt: now, updatedAt: now, createdBy: game.user.name, updatedBy: game.user.name, pendingDelete: false, hidden: hidden ?? false, comments: [], ...(game.user.isGM ? { gmNotes } : {}) });
    }
    const saved = await saveEntries(entries);
    if (saved === false) return;
    this._wikiApp.render(); this.close();
  }

  _broadcastEditingStop() {
    if (this._broadcastedStart && this._entry?.id) {
      this._broadcastedStart = false; activeEditors.delete(this._entry.id);
      game.socket.emit(SOCKET_EVENT, { action: "editingStop", entryId: this._entry.id, userId: game.user.id });
      refreshAllWikiApps();
    }
  }

  async close(options) {
    this._broadcastEditingStop(); this._editorReady = false;
...
class WikiDoodleEditor extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  async _insertDoodle() {
    if (!game.user.can("FILES_UPLOAD")) {
      ui.notifications.warn("Adventurer Wiki: You need file upload permissions to save doodles. Ask your GM to enable this in world settings.");
      return;
    }
    ...
    exportCanvas.toBlob(async (blob) => {
      ...
      try {
        ...
        ui.notifications.error("Adventurer Wiki: Failed to upload doodle. Check your file permissions and try again.");
      }
    }, "image/png");
  }
}
...
class WikiCategorySettings extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  constructor(options = {}) { super(options); this._working = foundry.utils.deepClone(getCategories()); }

  async _prepareContext() {
    const entries = getEntries();
    const categories = this._working.map((cat, idx) => ({ ...cat, count: entries.filter(e => e.category === cat.id).length, isFirst: idx === 0, isLast: idx === this._working.length - 1 }));
    return { categories };
  }
  ...
  _attachListeners() {
    if (this._listenersReady) return; this._listenersReady = true;
    const inApp = (e) => !!(this.element?.contains(e.target));
    const _click = async (e) => {
      if (!inApp(e)) return;
      const deleteBtn = e.target.closest(".wiki-cat-delete-btn:not([disabled])");
      if (deleteBtn) {
        const catId = deleteBtn.dataset.catId; const row = this.element.querySelector(`.wiki-cat-row[data-id="${catId}"]`);
        const label = row?.querySelector(".wiki-cat-label-input")?.value?.trim() ?? catId;
        const ok = await foundry.applications.api.DialogV2.confirm({ window: { title: "Delete Category" }, content: `<p>Remove the category "<strong>${label}</strong>"? This cannot be undone.</p>` });
        if (!ok) return; this._syncFromDOM(); this._working = this._working.filter(c => c.id !== catId); this.render({ force: true }); return;
      }
      const upBtn = e.target.closest(".wiki-cat-move-up");
      if (upBtn) {
        this._syncFromDOM(); const catId = upBtn.dataset.catId; const idx = this._working.findIndex(c => c.id === catId);
        if (idx > 0) { [this._working[idx - 1], this._working[idx]] = [this._working[idx], this._working[idx - 1]]; this.render({ force: true }); } return;
      }
      const downBtn = e.target.closest(".wiki-cat-move-down");
      if (downBtn) {
        this._syncFromDOM(); const catId = downBtn.dataset.catId; const idx = this._working.findIndex(c => c.id === catId);
        if (idx < this._working.length - 1) { [this._working[idx], this._working[idx + 1]] = [this._working[idx + 1], this._working[idx]]; this.render({ force: true }); } return;
      }
      if (e.target.closest(".wiki-cat-add-btn")) {
        const labelInput = this.element.querySelector(".wiki-cat-new-label"); const iconInput = this.element.querySelector(".wiki-cat-new-icon");
        const label = labelInput?.value?.trim(); const icon = iconInput?.value?.trim() || "fa-tag";
        if (!label) { ui.notifications.warn("Adventurer Wiki: Please enter a category name."); return; }
        const id = label.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "").replace(/-+/g, "-").replace(/^-|-$/g, "") || `cat-${Date.now()}`;
        if (this._working.find(c => c.id === id)) { ui.notifications.warn(`Adventurer Wiki: A category named "${label}" already exists.`); return; }
        this._syncFromDOM(); this._working.push({ id, label, icon });
        if (labelInput) labelInput.value = ""; if (iconInput) iconInput.value = ""; this.render({ force: true }); return;
      }
    ...
    const _input = (e) => {
      ...
      if (e.target.matches(".wiki-cat-icon-input")) {
        const row = e.target.closest(".wiki-cat-row"); const iconEl = row?.querySelector(".wiki-cat-preview-icon");
  ...
  _syncFromDOM() {
    ...
    for (const row of rows) {
      const catId = row.dataset.id; const cat = this._working.find(c => c.id === catId);
      if (!cat) continue;
      const label = row.querySelector(".wiki-cat-label-input")?.value?.trim();
      const icon  = row.querySelector(".wiki-cat-icon-input")?.value?.trim();
      if (label) cat.label = label; if (icon) cat.icon = icon;
    }
  }

  async _save() {
    this._syncFromDOM();
    if (this._working.length === 0) { ui.notifications.warn("Adventurer Wiki: You must keep at least one category."); return; }
    for (const cat of this._working) { if (!cat.label?.trim()) { ui.notifications.warn("Adventurer Wiki: All categories must have a name."); return; } }
    await game.settings.set(MODULE_ID, CATS_SETTING_KEY, this._working);
    game.socket.emit(SOCKET_EVENT, { action: "categoriesChanged" }); refreshAllWikiApps();
    ui.notifications.info("Adventurer Wiki: Categories saved."); this.close();
  }
